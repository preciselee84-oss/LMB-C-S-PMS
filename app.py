...
def build_integration_confirmation_docx_bytes(form_data):
    ...
    def set_table_borders(table, color="AAAAAA", size="6"):
    ...
    if erp_type == "더존 아마란스 10":
        ...
        for work_type in selected_work_types:
            for detail in AMARANTH_ATTACHMENT_TEMPLATES.get(work_type, []):
                ...
                set_cell_text(table.cell(0, 3), detail.get("transfer_method", ""))
                set_cell_text(table.cell(1, 2), "데이터 처리방식", True); set_cell_shading(table.cell(1, 2))
                set_cell_text(table.cell(1, 3), detail.get("process_method", ""))
                set_cell_text(table.cell(2, 2), "스케줄러 사용여부", True); set_cell_shading(table.cell(2, 2))
                set_cell_text(table.cell(2, 3), detail.get("scheduler", ""))
                ...
                set_cell_text(table.cell(5, 0), "비고", True); set_cell_shading(table.cell(5, 0))
                table.cell(5, 1).merge(table.cell(5, 3)); set_cell_text(table.cell(5, 1), "")
...
def show_integration_confirmation():
    ...
    st.download_button(
        "연계확인서 다운로드",
        data=build_integration_confirmation_docx_bytes(form_data),
        file_name=f"통합CMS_ERP연계작업결과서_{safe_customer}.docx",
...
def billing_template_value(df, row, candidates):
    for candidate in candidates:
        if candidate in df.columns:
            value = row.get(candidate, "")
            if isinstance(value, pd.Series):
                value = next((item for item in value.tolist() if billing_display_value(item)), "")
            return value
    return ""


def billing_template_df(source_df, column_specs):
...
def build_billing_template_download_dfs(parsed_open_sections, parsed_erp_sections, open_count, link_count, login_df, reference_lookup=None):
    reference_lookup = reference_lookup or {}
    prepared_open_sections = [
        (title, apply_billing_reference_to_source_df(df, reference_lookup))
        for title, df in parsed_open_sections
...
def render_billing_source_tables(source_upload=None, login_df=None):
    ...
    with download_col:
        st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
        if source_loaded:
            download_open_df, download_education_df, download_erp_df = build_billing_template_download_dfs(
                parsed_open_sections,
                parsed_erp_sections,
                open_count,
                link_count,
            ...
            st.download_button(
                "청구자료 엑셀 다운로드",
                data=billing_template_excel_bytes(download_open_df, download_education_df, download_erp_df),
